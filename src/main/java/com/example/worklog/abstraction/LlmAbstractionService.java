...
/**
 * OpenAI 互換エンドポイント経由でローカルLLM を呼ぶ実装。
 *
 * <p>Spring AI を使わず素の RestClient にしているのは、MVP 段階で必要なのが
 * 「1 回の非ストリーミング補完」だけであり、依存とバージョン制約を増やす利点が無いため。
 * RAG やベクトルストアが必要になる段階で Spring AI に載せ替える。
 */
@Service
public class LlmAbstractionService implements AbstractionService {

    private static final Logger log = LoggerFactory.getLogger(LlmAbstractionService.class);

    /** Qwen 系の思考モデルが出力する <think>...</think> を除去する。 */
    ...
    public LlmAbstractionService(LlmProperties properties) {
    ...
    public Optional<String> abstractText(String maskedText) {
        if (maskedText == null || maskedText.isBlank() || !isAvailable()) {
            return Optional.empty();
        }
        ChatRequest request = new ChatRequest(
                properties.model(),
                List.of(new Message("system", SYSTEM_PROMPT),
                        new Message("user", "以下の業務メモを抽象化してください。\n\n---\n" + maskedText + "\n---")),
        ...
        try {
            ...
            log.warn("抽象化に失敗しました。辞書マスキング結果をそのまま採用します: {}", e.toString());
